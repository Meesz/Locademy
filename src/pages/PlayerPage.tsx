...
function shouldMarkCompleted(
  ...
  if (
    settings.allowLastSecondsComplete &&
    duration - currentTime <= settings.lastSecondsWindow
  ) {
    return true;
  }
  return false;
...
export function PlayerPage() {
  ...
  useEffect(() => {
    setResumePrompt(
      (video?.progress?.lastPositionSec ?? 0) > RESUME_THRESHOLD_SECONDS &&
        !(video?.progress?.completed ?? false)
    );
  ...
  const handleLoadedMetadata = useCallback(async () => {
    ...
    const videoDuration = Number.isFinite(element.duration)
      ? element.duration
      : undefined;
    if (videoDuration) {
      ...
      if (
        !video.durationSec ||
        Math.abs(video.durationSec - videoDuration) > 1
      ) {
      ...
      const { posterBlobKey, durationSec } = await generateAndStoreThumbnail(
        thumbnailSource,
        {
          secondsIntoVideo: Math.min(
            ...
            videoDuration ? Math.max(videoDuration - 1, 2) : 6
          ),
        }
      );
  ...
  const handleTimeUpdate = useCallback(() => {
    ...
    if (
      ...
      shouldMarkCompleted(position, mediaDuration, settings)
    ) {
        ...
        <div className="space-y-4 rounded-3xl border border-slate-800 bg-slate-950/60 p-4 sm:p-6">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/70 p-6">
                <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-6 text-center">
                  <p className="text-sm text-slate-300">
                    Resume from {formatDuration(Math.floor(resumePosition))}?
                  </p>
          ...
          <div className="space-y-2">
            ...
            <Progress
              value={
                completed
                  ...
                  ? Math.min(
                      100,
                      (video.progress.lastPositionSec / duration) * 100
                    )
                  : 0
              }
            />
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
              ...
              {video.progress?.playCount ? (
                <span>• {video.progress.playCount} plays</span>
              ) : null}
            </div>
          </div>
        </div>
            ...
            <div className="space-y-2 text-xs text-slate-400">
              {recommendations.map((item) => (
                ...
                >
                  ...
                  <span>
                    {formatDuration(item.video.progress?.lastPositionSec ?? 0)}
                  </span>
                </Link>
              ))}
            </div>
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-4 text-sm text-slate-300">
          ...
          <p className="mt-2 text-xs text-slate-400">
            ...
            videos complete • {courseProgress.percent}%
          </p>
          <Progress value={courseProgress.percent} className="mt-3" />
          <div className="mt-4 grid gap-2 text-xs text-slate-400">
            <span>
              Total runtime ~ {formatDuration(courseProgress.totalDuration)}
            </span>
            <span>
              Watched ~{" "}
              {formatDuration(courseProgress.estimatedWatchedDuration)}
            </span>
            <button
              className={buttonClasses({
                variant: "ghost",
                className:
                  "mt-2 justify-center text-sky-300 hover:text-sky-200",
              })}
